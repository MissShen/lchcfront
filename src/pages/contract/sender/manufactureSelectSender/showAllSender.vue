<template>
  <div>
    <!--<div class="tabs-panel">-->
      <!--<span class="active">查看所有配送企业</span>-->
    <!--</div>-->
    <!-- 表格 -->
    <div class="tablebox">
      ...
      <div class="foot-btn">
        <div class="foot-left">
          <el-button type="primary" size="small" @click="submitIds()">签订委托协议</el-button>
      ...
      <div class="tableset">
        <el-table
          ...
          <el-table-column label="配送企业" prop="senderName" sortable="custom" :show-overflow-tooltip="true">
            <template slot-scope="scope">
              <span>{{ scope.row.senderName }}</span>
            </template>
          </el-table-column>
          <el-table-column label="已签订产品数量" prop="hasConcludedNum" sortable="custom">
            <template slot-scope="scope">
              <span>{{ scope.row.hasConcludedNum }}</span>
            </template>
          </el-table-column>
          <el-table-column label="未签订产品数量" prop="notConcludedNum" sortable="custom">
            <template slot-scope="scope">
              <span>{{ scope.row.notConcludedNum }}</span>
            </template>
          </el-table-column>
          <el-table-column label="两票制" prop="twoVoteId" sortable="custom">
            <template slot-scope="scope">
          ...
          <el-table-column label="配送评价" :show-overflow-tooltip="true" prop="SERVICESTATE" sortable="custom">
            <template slot-scope="scope">
              <span>
                <el-rate
          ...
          <el-table-column label="操作" width="180">
            <template slot-scope="scope">
              <div class="operates">
                <a href="javaScript:;" type="text" size="small" @click="showItem(scope.row.senderId)">查看明细</a>
              </div>
            </template>
          </el-table-column>
        </el-table>
    ...
    <!--  ====================================== 弹窗 ====================================== -->
    <!-- 点击查看明细弹框 -->
    <detail-view v-if="detailViewVisible" :parent-id="this.senderId" ref="detailView"></detail-view>
...
<script>
  ...
  // 引入用户相关请求
  import {queryAllSenderList, createAgreement} from 'src/axios/contract/sender/manufactureSelectSender'
  import {complainedTableQueryHttp,complainedDetailsHttp} from 'src/axios/contract/sender/hospitalSelectSender';
  import detailView from 'src/pages/contract/sender/manufactureSelectSender/showAllSender-detail'
  ...
  export default {
    ...
    methods: {
      ...
      list() {
        ...
        queryAllSenderList(this.resData, this.query).then(res => {
          console.log(res);
      ...
      showItem(senderId) {
        ...
        this.$nextTick(() => {
          // 通过组件中设置的ref属性调取detailView.vue中的fillData()方法加载弹窗内容
          this.$refs.detailView.fillData(senderId);
        });
      },
      // 重置用户表单
      resetForm() {
      ...
      complainedTableList(){
        console.log('---------------------');
        console.log(this.complainedTableQuery);
