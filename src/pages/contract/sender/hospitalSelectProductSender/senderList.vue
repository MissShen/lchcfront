<template>
  <div>
    ...
    <div class="tablebox">
          ...
          <el-table-column label="操作" width="170">
            <template slot-scope="scope">
              <div class="operates w100">
                <a href="javascript:;" class="blue-bord" @click="batchChoice(scope.row)">全选</a>
                <a href="javascript:;" class="green" @click="openUnselectedPage(scope.row.id)">选择产品</a>
              </div>
            </template>
          </el-table-column>
        </el-table>
      ...
      <div class="foot-btn">
        <div class="foot-left">
          ...
          <span><el-button type="warning" size="small" @click="Excel()">导出已选择的配送企业</el-button></span>
        </div>
      </div>
    ...
    <!-- 可选产品列表 -->
    <select-product-list v-if="selectedViewVisible" ref="selectedView"></select-product-list>
  </div>
</template>
<script>

  // 引入用户相关请求
  import {querySelectSenderList, querySelectedProductList, deleteCurrentSend,complainedTableQueryHttp,complainedDetailsHttp} from 'src/axios/contract/sender/hospitalSelectSender';
  import {generateContract, hospitalSelectProductSenderExcel, generateProductDetail} from 'src/axios/contract/sender/hospitalSelectProductSender'

  //  引入子页面
  import selectProductList from 'src/pages/contract/sender/hospitalSelectProductSender/selectProductList';
  ...
  export default {
    ...
    methods: {
      ...
      list() {
        ...
        querySelectSenderList(this.resData, this.query).then(res => {
      ...
      batchChoice(row){
        var senderIdListString = '';
        if(row != ''){
          senderIdListString = row.id;
        }else{
          var senderIds = [];
          for (var i = 0; i < this.mainMultipleSelection.length; i++){
            senderIds.push(this.mainMultipleSelection[i].id);
          }
          senderIdListString = senderIds.join(',').toString();
        }
        if(senderIdListString != null && senderIdListString != ''){
          generateProductDetail(senderIdListString).then(res => {
            this.$message({
              message: res.success == true ? res.data : res.message,
              type: res.success == true ? "success" : "error",
              onClose: () => {
        ...
        /*selectAllProduct(row.senderId).then(res => {
          this.$message({
            message: res.success == true ? "选择成功" : "选择失败",
            type: res.success == true ? "success" : "error",
            onClose: ()=> {this.list()}
          })
        })*/
