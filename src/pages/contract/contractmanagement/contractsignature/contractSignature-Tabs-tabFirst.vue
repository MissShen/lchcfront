    ...
    <div class="tablebox">
      <!-- 表格查询表单 -->
        <el-form ref="query" :model="query" size="small" :inline="true" class="table-form">
            <el-form-item>
              <el-select v-model="query.conType" size="small" placeholder="合同名称">
                <el-option-group
                  key="合同名称"
                  label="合同名称">
                  <el-option
          ...
          <el-table-column label="甲方签章时间" prop="jia_Sign_Date" sortable="custom" :show-overflow-tooltip="true">
            <template slot-scope="scope">
              <el-tooltip :enterable="false" class="item" effect="dark" placement="top-start">
                ...
                <span v-if="scope.row.jiaSignDate!=null && scope.row.yiSignDate!=''">{{ moment(scope.row.jiaSignDate).format("YYYY-MM-DD") }}</span>
                <span v-else></span>
              </el-tooltip>
            </template>
          </el-table-column>
...
<script>
  ...
  // 引入用户相关请求  引自js文件
  import {selectContractList,getContractSignatureMainState} from 'src/axios/contract/contractsignature/contractList';
  ...
  export default {
    ...
    methods: {
      ...
      list () {
        ...
        selectContractList(this.resData, this.query).then(res => {
          console.log(res);
