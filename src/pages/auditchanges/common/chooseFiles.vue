  ...
  <div class="zbl_treebox">
    <el-dialog
      ...
      <div slot="footer" class="dialog-footer">
        <el-button @click="emptyTree" type="danger">清除选择</el-button>
        <el-button @click="toFileManage" type="warning">选择文件</el-button>
...
//              this.$message.error(res.data.exceptionMessage);
      ...
      renderContent(h, { node, data, store }) {
        if(data.type==null){
          return (<span class='choose-file'><span class='blue iconfont icon-wenjian'></span><span>{node.label}</span></span>);
        }else if(data.type=='.jpg'||data.type=='.png'||data.type=='.jpeg'){
          return (<span class='choose-picture'><span class="red el-icon-picture "></span><span >{node.label}</span></span>);
        }else{
          return (<span><span class="el-icon-document"></span><span >{node.label}</span></span>);
        };
      },
      toFileManage(){
