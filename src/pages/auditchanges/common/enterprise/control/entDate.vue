<template>
  <el-form-item :label="label" :prop="prop">
    ...
    <div>
      ...
      <a class="audit-answer-if" type="success right" v-if="btn && cli" @click="faq">{{btn}}</a>
    </div>
    <div v-if="after" class="orange">变更后：{{after}}</div>
    <div v-if="audit" class="orange">审核后：{{audit}}</div>
    <el-date-picker v-if="limit" v-model="currentValue" :placeholder="label"></el-date-picker>
...
<script>
  ...
  export default {
    props: ["label", "prop", "before", "after", "audit", "btn", "form", "limit", "cli", "upload", "fileList", "field"],
    components: {
