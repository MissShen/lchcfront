<template>
  ...
  <div class="tab-box">
    <!--<div class="tabs-panel">
      <router-link to="/AUDIT/orgAffirm/productMessageDeclareAffirm">产品信息申报确认</router-link>
      <span class="active">产品信息变更确认</span>
    </div>-->
          ...
          <template slot-scope="scope">
            <div class="operates">
              <span v-if="scope.row.STATE == '8'">
                <a class="operation-btn" @click="handleGoAffirm(scope.row.ID,scope.row.UPPRODUCTID)">去确认</a>
              </span>
              <span v-if="scope.row.STATE == '9'">
                <a class="operation-btn" @click="goDetail(scope.row.ID,scope.row.UPPRODUCTID)">详情</a>
              </span>
            </div>
          </template>
        </el-table-column>
    ...
    <change-affirm ref="changeAffirm"></change-affirm>
...
<script>
  import {queryListChange} from 'src/axios/auditchanges/orgAffirm/productAffirm'
  // import {keyBoardEnter} from "src/utils";
  import changeAffirm from "./changeAffirm"
  ...
  export default {
    ...
    methods: {
      ...
      handleGoAffirm(id,upProductId){
        this.$nextTick(() => {
          this.$refs.changeAffirm.list(id,upProductId)
        })
      },
      goDetail(id,upProductId){
        this.$nextTick(() => {
          this.$refs.changeDetail.list(id,upProductId)
        })
      },
      // ************************
      ...
      toDeclare(t) {
        if(t.name == "first") {
          this.$router.push('/AUDIT/orgAffirm/productMessageDeclareAffirm');
        }
      },
