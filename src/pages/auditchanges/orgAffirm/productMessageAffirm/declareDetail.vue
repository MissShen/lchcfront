  ...
  >
    ...
    <div class="main audit-main">
      <ul class="side side-init" id="product-affirm-view-side">
        <li v-for="(item,index) in nodesPosition" :key="index"><a @click="changeScrollHeight(item.nodePosition,'product-affirm-view-box','jump-pro-affirm-view',index)" class="current d_jump  jump-pro-affirm-view">{{item.itemName}}</a></li>
      </ul>
      <div class="right-box">
        <product-info  id="product-affirm-view"  ref="productInfo"></product-info>
        <div class="audit-info">
          <el-form :model="dataForm" ref="showForm" :rules="rules" label-width="125px" size="small" label-position="left">
            <h3 id="affirm-jgxx">产品价格信息</h3>
            <el-row :gutter="24">
              <el-col :span="12">
                <el-form-item class="audit-declare-radio" prop="PRICETYPE">
                  <el-radio v-model="showForm.PRICETYPE" label="0">全国最低中标价</el-radio>
                  <el-radio v-model="showForm.PRICETYPE" label="1">计算价格</el-radio>
                  <el-radio v-model="showForm.PRICETYPE" label="2">无最低中标价</el-radio>
                  <span v-if="auditForm.PRICETYPE"><br/>审核后:{{auditForm.PRICETYPE}}</span>
                </el-form-item>
              </el-col>
            </el-row>
...
<script>
  ...
  import {findProductDetailById,selectQueryFieldById,updaterefuse,insert} from 'src/axios/auditchanges/orgAffirm/productAffirm'
