<template>
  ...
  <div class="tab-box">
    <!--<div class="tabs-panel">-->
      <!--<span class="active">产品价格申报澄清</span>-->
      <!--<router-link to="/AUDIT/orgClarify/priceMessageChangeClarify">产品价格变更澄清</router-link>-->
    <!--</div>-->
   ...
   <!-- <el-tabs v-model="activeName" @tab-click="toDeclare">
      ...
      <div class="tablebox">
          <el-form ref="query" :model="query" label-position="right" size="small" :inline="true" class="table-form">
            <el-form-item>
              ...
              <el-form-item>
              <el-input class="width-150" v-model="query.productName" placeholder="产品名称" @keyup.enter.native="searchEnterFun" size="small"></el-input>
            </el-form-item>
              <el-form-item>
              <el-input class="width-150" v-model="query.dosage" placeholder="剂型" @keyup.enter.native="searchEnterFun" size="small"></el-input>
            </el-form-item>
              <el-form-item>
              <el-input class="width-150" v-model="query.specification" placeholder="规格" @keyup.enter.native="searchEnterFun" size="small"></el-input>
            </el-form-item>
              <el-form-item>
        ...
        <div class="tableset">
          ...
          >
           ...
           <!-- <el-table-column label="产品名称">
            ...
            <el-table-column label="转换比" width="80" header-align="left" align="right">
              <template slot-scope="scope">
            ...
            <el-table-column label="澄清记录">
              <template slot-scope="scope">
              <!--  <span>{{scope.row.clarifyCount}}</span>-->
              <!--     <span v-if="scope.row.clarifyCount==scope.row.questionCount">已澄清</span>
                <span v-if="scope.row.clarifyCount!=scope.row.questionCount">未澄清</span>-->
            ...
            <el-table-column label="操作" width="100">
              <template slot-scope="scope">
                <div class="operates">
                  <span>
                    <a class="operation-btn" @click="questionHandle(scope.row.projectProductId)">澄清</a>
                  </span>
                  <!--<span>-->
                    <!--<a class="operation-btn" @click="detailHandle(scope.row.projectProductId)">详情</a>-->
                  <!--</span>-->
                </div>
              </template>
            </el-table-column>
    ...
    <priceMessageClarify v-if="clarifyShow" ref="priceMessageClarify"></priceMessageClarify>
  </div>
</template>
...
<script>
  ...
  // 引入用户相关请求
  import {priceDeclareList} from 'src/axios/auditchanges/messageClarify/price'
  // import {keyBoardEnter} from "../../../../utils";
  import priceMessageClarify from './priceMessageClarify'
  export default {
    components:{
      priceMessageClarify
    },
    ...
    methods: {
      ...
      // ------------------------------------------------- 处理方法 ----------------------------------------
      list(){
        priceDeclareList(this.resData,this.query).then(res =>{
     ...
     /* toDeclare(t) {
      ...
      questionHandle(id) {
        ...
        this.$nextTick(() => {
          this.$refs.priceMessageClarify.list(id)
        })
      },
      detailHandle(id) {
        this.clarifyShow = true;
        this.$nextTick(() => {
          this.$refs.priceMessageClarify.list(id,true)
        })
      }
    }
  }
