<template>
  <el-dialog
    ...
    append-to-body
    class="audit-dialog"
    id="product-clarify-view-box"
  >
    <div v-if="visible">
    <div class="main audit-main">
      <ul class="side side-init" id="product-clarify-view-side">
        <li v-for="(item,index) in nodesPosition" :key="index"><a @click="changeScrollHeight(item.nodePosition,'product-clarify-view-box','jump-pro-clarify-view',index)" class="d_jump jump-pro-clarify-view">{{item.itemName}}</a></li>
      </ul>
      <div class="right-box">
        <product-info-detail id="product-clarify-view" ref="productInfoDetail"></product-info-detail>
      </div>
    </div>
    </div>
...
<script>
  ...
  import {findProductById,queryProductChangeAfter,findChangeAttachedFile,findAttachedFiles} from 'src/axios/auditchanges/messageChanges/productChange'
  import {checkJydate,returnAnnex,returnAnnexFile,returnDataAnnex} from 'src/axios/auditchanges/common/catProduct'
  import {selectQueryFieldById} from 'src/axios/auditchanges/messageClarify/productClarify'
...
//        this.flagForm//质疑数据
       ...
       /* this.visible = true;
        this.showForm.CLARIFYFLAG = true;
        this.showForm.CHANGFLAG = true;
        this.$nextTick(() => {
          this.$refs.productInfoDetail.list(this.showForm,{},this.changeForm,this.flagForm,{})
        })*/
        this.visible = true;
        this.$nextTick( async () => {
          ...
          this.annex = returnAnnex(this.showForm.ATTACHED,{},this.changeForm,{},this.flagForm);
          ...
          await this.$refs.productInfoDetail.list(this.showForm,{},this.changeForm,this.flagForm,{},{},this.annex,this.attachedFileList);

          /**
           * 查询设置锚点位置
           * */
          //恢复初始状态
          this.removeClassCurrent('jump-pro-clarify-view')[0].classList.add('current');
          this.getNodesHeight('product-clarify-view','shxx-pro-clarify-view')
        })
    ...
    watch:{
      visible(newVal){
        let vieNode = document.getElementById('product-clarify-view-box');
        if(vieNode){
          if(newVal == false){
            vieNode.onscroll = null;
            this.removeFixed('product-clarify-view-side')
          }else{
            vieNode.onscroll = function () {
              this.dialogBoxScroll(vieNode,'jump-pro-clarify-view','product-clarify-view-side')
            }.bind(this)
          }
        }
      }
