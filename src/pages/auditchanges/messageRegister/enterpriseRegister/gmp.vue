    ...
    <div style="width: 100%">
      <el-form ref="form" :model="form" :rules="rules" :disabled="state == 1" label-width="120px">
        ...
        <el-form-item label="上传附件" prop="fileIdList">
          <el-button @click="handleChooseFiles('gmp')" :disabled="state == 1">选择文件</el-button>
        </el-form-item>
        <el-form-item>
          <images-preview :operate="state != 1" ref="imagesPreview" refName="gmp" v-if="form.fileIdList" :imgIdListUnUnique="form.fileIdList" @imgIdListChange="upLoadImgIdList" />
        </el-form-item>
      </el-form>
      <choose-files ref="treeView" v-if="dialogVisible"  @handleSelectedFolder="selectedData"></choose-files>
    </div>
    <span slot="footer">
      <el-button type="primary" @click="handleForm" :disabled="state == 1" size="small">保存</el-button>
    ...
    methods: {
      ...
      selectedData(res){
        ...
        if(!this.form.fileIdList) {
          this.$set(this.form, "fileIdList", []);
        }
        this.$nextTick(()=>{
          res.data.forEach(function (item) {
            this.form.fileIdList.push(item);
          }.bind(this));
        })
      },
      upLoadImgIdList(res){
        ...
        console.log('========从预览里面来的');
        console.log(res);
        this.$set(this.form, "fileIdList", res.data);
      },
    }
  }
</script>
