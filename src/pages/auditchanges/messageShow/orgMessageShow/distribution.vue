...
<script>
  import {queryDistributionList} from 'src/axios/auditchanges/infoshow/orgInfoShow'
  ...
  export default {
    ...
    methods: {
      ...
      // ------------------------------------------------- 处理方法 ----------------------------------------
      list () {
        queryDistributionList(this.resData, this.query).then(res => {
          console.log(res);
      ...
      handleView(id) {
        this.$nextTick(() => {
          this.$refs.distributionPage.list(id)
        })
      }
    }
  }
