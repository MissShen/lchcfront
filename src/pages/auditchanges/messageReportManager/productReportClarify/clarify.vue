<template>
  ...
  <div>
    ...
    <!--<div class="main audit-main">-->
          <el-form ref="dataForm" :model="dataForm" :rules="dataRule" label-width="130px" @keyup.enter.native="submit()" size="small" label-position="left">
            <el-row>
                <el-col :span="12">
            ...
            <el-row>
              <el-col :span="24">
                <el-form-item label="澄清内容" prop="CLARIFYEXPLAN">
                  <el-input type="textarea" :rows = "3" v-model="dataForm.CLARIFYEXPLAN" placeholder="请输入澄清内容" class="w80"></el-input>
                </el-form-item>
              </el-col>
            </el-row>
            <el-row>
...
import {findReportById,updateClarify} from 'src/axios/auditchanges/messageReportManager/messageReport/productReport'
  ...
  export default {
    ...
    methods: {
      ...
      metaRule() {
        return {
          CLARIFYEXPLAN: [
            { required: true, message: '澄清内容不能为空', trigger: 'blur' }
      ...
      submit() {
       this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$confirm('确认澄清不可修改，是否确定完成此操作?', '提示', {
              confirmButtonText: '确定',
              cancelButtonText: '取消',
              type: 'warning'
            }).then(() => {
              this.dataForm.REPORTID = this.reportId;
              console.log('测试更新')
              updateClarify(this.dataForm,this.fromUploadIdList)
              // this.$router.go(-1)
              this.$emit('update')
              this.visible = false;
            })
