  ...
  <div class="tablebox">
    ...
    <div class="tableset">
      <el-table
        ...
        <el-table-column header-align="left" align="right" label="转换比" width="60px">
          <template slot-scope="scope">
        ...
        <el-table-column label="生产企业" width="180px">
          <template slot-scope="scope">
            <span>{{ scope.row.MANUFACTURE }}</span>
          </template>
        </el-table-column>
        <el-table-column label="质疑字段">
          <template slot-scope="scope">
        ...
        <el-table-column label="操作" width="80px">
          <template slot-scope="scope">
            <div class="operates">
              <span>
                <a class="operation-btn" @click="handleGoDoubt(scope.row.ID,scope.row.QTID)">详情</a>
              </span>
            </div>
          </template>
        </el-table-column>
...
<script>
  import {findProjectProductPrices} from 'src/axios/auditchanges/messageDoubt/priceDoubt'
  ...
  export default {
    ...
    methods: {
      ...
      list () {
        this.$store.state.pageSeach=this.query;
        findProjectProductPrices(this.resData, this.query).then(res => {
