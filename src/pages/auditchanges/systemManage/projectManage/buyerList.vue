<template>
  <el-dialog
    ...
    append-to-body
  >
    <el-form ref="query" :model="query" label-width="150px" :inline="true" size="small" class="table-form">
        <el-form-item>
          <el-input v-model="query.name" placeholder="企业名称" size="small"></el-input>
        </el-form-item>
        <el-form-item>
          <el-input v-model="query.createCode" placeholder="社会统一信用代码" size="small"></el-input>
        </el-form-item>
        <el-form-item>
          <el-input v-model="query.organizeCode" placeholder="组织机构代码" size="small"></el-input>
        </el-form-item>
        <el-form-item>
    ...
    <div class="tableset">
      <el-table :data="resData.list" ref="tableData" @selection-change="handleSelectionChange" border stripe :row-key="getRowKeys">
...
<script>
  import {getAllBuyer} from 'src/axios/auditchanges/systemManage/orgList'
  ...
  export default {
    ...
    methods: {
      ...
      handleSelectionChange(val) {
        ...
        // sessionStorage.setItem('buyerList'+key, val);
        //this.cacheData.push({key: val})
        //this.multipleSelection = val;
      },
      selectTabs() {
        // alert(123);
        let arr = [];
        for(let i=0; i<this.resData.pages; i++){
          if(sessionStorage.getItem('buyerList'+i)){
            arr.push(sessionStorage.getItem('buyerList'+i));
          }
        }
        // console.log(arr,'arr');
        let selectedDataId =this.selectedData.map(item=>item.id);
        // console.log(selectedDataId,'selectedDataId')
        this.$emit('selectedId',selectedDataId);
        this.visible=false;
        console.log(this.selectedData,'this.selectedData')
      ...
      select() {
        ...
        // alert(selectQuery.name);
        // alert(selectQuery.createCode);
        // alert(selectQuery.organizeCode);
        // this.visible=false;
        ...
        getAllBuyer(resData, selectQuery).then(res => {
          if (res.success) {
            ...
            for(let i=0; i<res.data.list.length; i++){
              let ids = _this.buyId.some(item=>item==res.data.list[i].id);
              if(ids){
