<template>
  <div class="project-manage-wrapper">
    <!--<el-tab-pane label="新增项目管理" name="first">-->
      <div class="tablebox">
          <el-form ref="query" class="form-bold" :model="query" label-position="left" size="small" label-width="120px" :rules="rules">
            <el-row>
              <el-col :span="10">
            ...
            <el-row :gutter="24">
              <el-col :span="24">
                <el-form-item>
                  <el-button type="primary" @click="submitForm('query')" >保存</el-button>
      ...
      <buyer-list v-if="buyerList" ref="buyerList" @selectedId="selectedId"></buyer-list>
      <seller-list v-if="sellerList" ref="sellerList" @selectedSellerId="selectedSellerId"></seller-list>
...
<script>
  ...
  import {submitFrom,validationProjectName,validationProjectCode} from 'src/axios/auditchanges/systemManage/projectManage'
  ...
  export default {
    ...
    data() {
      ...
      return {
        ...
        endPickerOptions: {
          disabledDate(time) {
            return time.getTime() < (new Date(that.query.beginDate*1))*1+3600*24*1000;
          }
        },
      }
    },
        ...
        if(this.requiredControl==false){//否
          this.$refs[formName].validate((valid) => {
            if (valid) {
              ...
              submitFrom(paramsForm).then(res=>{
                // console.log(res,'res');
