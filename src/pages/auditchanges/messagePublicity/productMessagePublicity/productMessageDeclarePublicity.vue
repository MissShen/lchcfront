<template>
  ...
  <div class="tab-box">
    <!--<div class="tabs-panel">
      <span class="active">产品信息申报公示</span>
      <router-link to="/AUDIT/messagePublicity/productMessageChangePublicity">产品信息变更公示</router-link>
    </div>-->
          ...
          <template slot-scope="scope">
            <div class="operates">
              <span v-if="scope.row.STATE == '3' && scope.row.QUERYFLAG == '0'">
                <a class="operation-btn" @click="handleGoQuery(scope.row.ID)">质疑</a>
              </span>
              <span>
                <a class="operation-btn" @click="handleGoView(scope.row.ID)">详情</a>
              </span>
            </div>
          </template>
        </el-table-column>
    ...
    <declare-find-view ref="declareFindView"></declare-find-view>
    <declare-query ref="declareQuery"></declare-query>
  </div>
    </div>
  </div>
</template>
<script>
  import {queryList} from 'src/axios/auditchanges/messagePublicity/productPublicity'
  import {keyBoardEnter} from "src/utils";
  import declareQuery from './declareQuery'
  import declareFindView from "./declareFindView";
  export default {
    components:{
      declareQuery,
      declareFindView
    ...
    methods: {
      ...
      handleGoView(id){
        this.$nextTick(() => {
          this.$refs.declareFindView.list(id)
        })
      },
      ...
      toChange(t) {
        if(t.name == "second") {
          this.$router.push('/AUDIT/messagePublicity/productMessageChangePublicity');
        }
      },
      handleGoQuery(id){
        this.$nextTick(() => {
          this.$refs.declareQuery.list(id)
        })
      }
    }
  }
