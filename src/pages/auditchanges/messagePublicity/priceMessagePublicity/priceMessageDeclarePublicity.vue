<template>
  <!-- 表格 -->
    <div class="tablebox">
        ...
        <el-table :data="resData.list" border stripe>
          ...
          <el-table-column label="操作" width="140px">
            <template slot-scope="scope">
              <div class="operates">
                <span>
                    <a class="operation-btn" v-if="scope.row.status == '3' && scope.row.queryFlag == '0'" @click="questionHandle(scope.row.id)">质疑</a>
                </span>
                <span>
                  <a class="operation-btn" @click="detailHandle(scope.row.id)">详情</a>
                </span>
              </div>
            </template>
          </el-table-column>
      ...
      <price-publicity-detail ref="pricePublicityDetail"></price-publicity-detail>

      <price-publicity-doubt ref="pricePublicityDoubt"></price-publicity-doubt>
    </div>
</template>
...
<script>
  ...
  // 引入用户相关请求
  import {findProjectProducts} from 'src/axios/auditchanges/messagePublicity/price'
  import {keyBoardEnter} from "../../../../utils";
  import pricePublicityDetail from './pricePublicityDetail'
  import pricePublicityDoubt from './pricePublicityDoubt'

  export default {
    components: {
      pricePublicityDetail,
      pricePublicityDoubt
    },
    data() {
      return {
    ...
    methods: {
      ...
      list() {
        this.$store.state.pageSeach=this.query;
        findProjectProducts(this.resData, this.query).then(res => {
     ...
     /* //查询
      ...
      questionHandle(id) {
        this.$nextTick(() => {
          this.$refs.pricePublicityDoubt.list(id)
        })
      },
      detailHandle(id) {
        this.$nextTick(() => {
          this.$refs.pricePublicityDetail.list(id)
        })
      }
    }
  }
