<template>
  <el-dialog
    ...
    append-to-body
    class="audit-dialog"
    id="price-change-detail-box"
  >
  <div v-if="visible" class="main audit-main">
    <ul class="side side-init" id="product-price-side">
      <li v-for="(nodeItem,index) in nodesPosition" :key="index"><a @click="changeScrollHeight(nodeItem.nodePosition,'price-change-detail-box','jump-price',index)" class="d_jump jump-price">{{nodeItem.itemName}}</a></li>
    </ul>
    <div class="right-box">
      <div class="tableset audit-info">
        <el-form ref="showForm" id="price-change-detail" label-width="150px" size="small" label-position="left">
          <h3 class="cpjc">产品基础信息</h3>
          <price-basics  :projectProductId="this.projectProductId" :changeFlag="true"></price-basics>
          ...
          <div v-if="showForm[0].CATALOGAREA == '2' || showForm[0].CATALOGAREA == '3'">
            ...
            <el-row :gutter="24" class="audit-item-box">
          ...
          <div v-if="changeForm[0].CATALOGAREA == '2' || changeForm[0].CATALOGAREA == '3'">
            ...
            <el-row class="audit-item-box" :gutter="2" v-for="(item,index) in changeForm" :key="index">
              <el-col :span="5">
                <el-form-item class="acc-price-label" label="省份：">
          ...
          <span><h3>项目名称应包括项目年度和省份名称，请按照各省公布的标准项目录名称录入。   最低价请如实、谨慎填写，注意不同规格包装间的合理比价关系！</h3></span>
        </el-form>
      </div>
    </div>
    ...
    components:{
      ...
      methods:{
        ...
        async list(id,productId){
          this.visible = true;
          this.projectProductId = productId;
          await findShowPriceByProductId(productId).then(res=>{
             ...
             * */
            //恢复初始状态
            this.removeClassCurrent('jump-price')[0].classList.add('current');
            this.getNodesHeight('price-change-detail')
          })
      ...
      watch:{
        visible(newVal){
          let vieNode = document.getElementById('price-change-detail-box');
          if(vieNode){
            if(newVal == false){
              vieNode.onscroll = null;
              this.removeFixed('product-price-side')
            }else{
              vieNode.onscroll = function () {
                this.dialogBoxScroll(vieNode,'jump-price','product-price-side')
              }.bind(this)
            }
          }
        }
