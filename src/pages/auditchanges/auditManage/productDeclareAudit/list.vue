<template>
  <div class="tab-box">
    ...
    <!--<div class="iregular-tabs">-->
      <!--<span class="is-active el-tabs__item">产品信息申报审核</span>-->
      <!--<span class="el-tabs__item"><router-link to="/AUDIT/auditManage/productChangeAuditList">产品信息变更审核</router-link></span>-->
    <!--</div>-->
      ...
      <div class="tablebox">
        ...
        <div class="tableset">
          ...
          >
            ...
            <el-table-column label="转换比" align="right" header-align="left" width="60px" show-overflow-tooltip>
              <template slot-scope="scope">
            ...
            <el-table-column label="审核结果">
              <template slot-scope="scope">
                <span>{{{"0":"未审核","1":"审核通过","2":"待确认","3":"审核不通过"}[scope.row.AUDITRESULT]}}</span>
              </template>
            </el-table-column>
            <el-table-column label="操作" width="120px">
              <template slot-scope="scope">
                <div class="operates">
                  <span v-if="scope.row.STATE == '5' && scope.row.AUDITFLAG =='1'"><a  class="operation-btn" @click="fromAudit(scope.row.ID)">审核</a></span>
...
<script>
  ...
  import {queryList,queryProjectCheckCount} from 'src/axios/auditchanges/auditManage/productDeclareAudit'
  ...
  export default {
   ...
   /* //导航守卫 监测到路由变化后 删除本地搜索条件
    ...
    methods: {
      toChange(t) {
        if(t.name == "second") {
          this.$router.push('/AUDIT/auditManage/productChangeAuditList');
        }
      },
      // -------------------------------------------- 数据初始化  -----------------------------------------
      metaData() {
