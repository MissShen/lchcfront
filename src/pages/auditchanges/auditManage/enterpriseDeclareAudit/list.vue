  ...
  <div class="tablebox">
    <!-- 表格查询表单 -->
      <el-form ref="query" :model="query" label-position="right" size="small" class="table-form" :inline="true">
          ...
          <el-form-item>
            <el-input v-model="query.CREDIT_CODE" placeholder="社会统一信用代码" @keyup.enter.native="searchEnterFun"
                      size="small"></el-input>
          </el-form-item>
          <el-form-item>
            <el-input v-model="query.ORGANIZE_CODE" placeholder="组织机构代码" @keyup.enter.native="searchEnterFun"
                      size="small"></el-input>
          </el-form-item>
          <el-form-item>
            <el-select v-model="query.CHECK_COUNT" placeholder="全部" size="small">
              <el-option key="0" label="全部" value=""/>
              <el-option v-for="x in checkCount" :key="x + 1" :label="{
                '1':'一',
                '2':'二',
                '3':'三',
                '4':'四',
    ...
    <div class="tableset">
      <el-table :data="resData.list" border stripe @selection-change="handleSelectionChange" :row-key="getRowkeys">
        ...
        <el-table-column label="申报时间" :formatter="dateformat" prop="CREATE_DATE" width="100px"/>
        ...
        <el-table-column label="操作" width="120px">
          <template slot-scope="scope">
            <div class="operates">
              <span><a href="javascript:;" class="operation-btn" @click="handleGoView(scope.row.ID)">详情</a></span>
              <span><a href="javascript:;" class="operation-btn"
                 v-if="scope.row.STATE == '5' && scope.row.AUTH == '1'"
                 @click="handleGoAudit(scope.row.ID, scope.row.CHECK_COUNT, scope.row.AUTH)">审核</a></span>
            </div>
          </template>
        </el-table-column>
      </el-table>
    ...
    >
      <div class="main audit-main">
        ...
        <div class="right-box">
          <div class="gap-40">
            <info id="product-addup" ref="info"></info>
            <el-form ref="form" :model="audit" :rules="rules" label-position="right" size="small" label-width="160px">
              ...
              <el-row :gutter="24">
                ...
                <el-col :span="18" v-if="form.CHECK_COUNT">
                  <el-form-item label="审核结果" prop="CHECK_RESULT">
                ...
                <el-col :span="18" v-if="form.CHECK_COUNT">
                  <el-form-item label="审核备注" prop="CHECK_MARK">
...
<script>
  ...
  import {
    ...
    declareSave,
    recording
  } from "src/axios/auditchanges/auditManage/enterprise";
  import {decodeQuestion, decodeAudit, checkControl} from "src/axios/auditchanges/common/nestedJson";
  ...
  export default {
    ...
    methods: {
      ...
      //  获取用户分页列表
      list() {
        declareList(this.resData, this.query).then(res => {
          console.log(res);
      ...
      handleForm() {
        this.$refs.form.validate(async valid => {
          if (valid) {
            ...
            // 转换级联框，需要特殊处理
            if (area) {
              this.form.enterprise.COUNTRY_ID = area[0];
              this.form.enterprise.PROVINCE_ID = area[1];
              this.form.enterprise.CITY_ID = area[2];
              this.form.enterprise.COUNTY_ID = area[3];

              show.enterprise.COUNTRY_ID$ = !!area[0];
              show.enterprise.PROVINCE_ID$ = !!area[1];
              show.enterprise.CITY_ID$ = !!area[2];
              show.enterprise.COUNTY_ID$ = !!area[3];
            }

            delete this.form.region;

            let res = await declareSave(
              this.form,
              this.$refs.info.show,
              this.audit,
              this.count,
      ...
      handleGoView(id) {
        ...
        this.auth = false;
        this.$nextTick(async () => {
          let old = await declare(id);

          this.doubt = await doubt(old.data.org.ID);
          this.clari = await clarify(old.data.org.ID);
      ...
      handleGoAudit(id, count, auth) {
        ...
        this.$nextTick(async () => {
          ...
          this.doubt = await doubt(old.data.org.ID);
          this.clari = await clarify(old.data.org.ID);
          let _audit = await audit(old.data.org.ID);
