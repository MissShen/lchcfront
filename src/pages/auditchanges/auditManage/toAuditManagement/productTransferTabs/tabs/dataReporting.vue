  ...
  <div>
      ...
      <!-- 表格查询表单 -->
        <el-form ref="query" :model="query" label-position="right" size="small" :inline="true" class="table-form">
          ...
          <el-form-item class="width-160">
                <el-input v-model="query.requestOrg " placeholder="申请企业名称" size="small" @keyup.enter.native="searchEnterFun"></el-input>
...
<script>
  //引入列表数据
  import {productChangeBatchAdd,upProductTransferInsert,upProductTransferCount,upProductTransferInsertAll} from 'src/axios/auditchanges/toAuditManagement/productTransferTabs'
...
//      this.list()
    ...
    methods: {
      ...
      list() {
        ...
        if (resData.pageNum == undefined || resData.pageNum == "") {
          resData.pageNum = 1;
            ...
            productChangeBatchAdd(resData,selectQuery).then(res=>{
      ...
      selectCreatedBatch(selectedDataId){//选择创建批次
        ...
        upProductTransferInsert(idForm).then(res=>{
          // console.log(res,'res......')
          if(res.success){
            this.$message({
              message:'成功',
      ...
      allBatch(){//全选创建批次
        upProductTransferCount().then(res=>{
          // console.log(res,'res......')
          if(res.success){
      ...
      allCreatedBatch(){//全选创建批次
        upProductTransferInsertAll().then(res=>{
          // console.log(res,'res......')
          if(res.success){
            this.$message({
              message:'保存成功',
