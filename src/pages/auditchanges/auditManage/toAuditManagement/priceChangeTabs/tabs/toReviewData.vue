...
<script>
  import dataReporting from 'src/pages/auditchanges/auditManage/toAuditManagement/priceChangeTabs/tabs/dataReporting'
  //引入列表数据
  import {priceToChangeList} from 'src/axios/auditchanges/toAuditManagement/priceChangeTabs'
  ...
  export default {
    ...
    //初始化函数
    created() {
      if(this.$store.state.resDataObj['6']){
        this.resData = this.$store.state.resDataObj['6']
      }
      //初始化列表
      this.select()
    },
    methods: {
      ...
      select() {
        ...
        priceToChangeList(resData).then(res=>{
      ...
      handleSizeChange(val) {
        ...
        this.select();
        this.$store.state.resDataObj['6'] = this.resData;
      ...
      setProjectNum(rowId,rowName,processType){//设置批次数量
        ...
        this.$router.push('priceChangeTabsBatchSet/'+rowId+'/'+rowName+'/'+processType)
      },
      showAuditor(rowId){//审核员分配
        this.$router.push('priceChangeTabsDistribution/'+rowId)
      },
