...
<script>
  //引入列表数据
  import {priceChangeBatchAdd,priceToChangeInsert,priceToChangeCount,priceToChangeInsertAll} from 'src/axios/auditchanges/toAuditManagement/priceChangeTabs'
...
//      this.list()
    ...
    methods: {
      ...
      list() {
        ...
        priceChangeBatchAdd(resData,selectQuery).then(res=>{
      ...
      selectCreatedBatch(selectedDataId){//选择创建批次
        ...
        priceToChangeInsert(idForm).then(res=>{
          // console.log(res,'res......')
          if(res.success){
            this.$message({
              message:'成功',
      ...
      allCreatedBatch(){//全选创建批次
        priceToChangeInsertAll().then(res=>{
          // console.log(res,'res......')
          if(res.success){
            this.$message({
              message:'保存成功',
