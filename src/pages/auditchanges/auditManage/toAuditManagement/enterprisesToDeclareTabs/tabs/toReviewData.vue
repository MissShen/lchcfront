        ...
        <el-table :data="resData.list" border stripe >
          ...
          <el-table-column label="批次数量" header-align="center" align="right">
            <template slot-scope="scope" >
              ...
              <!--<el-button type="text" size="small" @click="setProjectNum(scope.row.id)">123412</el-button>-->
            </template>
          </el-table-column>
          <el-table-column label="审核员分配情况">
            <template slot-scope="scope">
              <a @click="showAuditor(scope.row.id)">{{ scope.row.auditorAllotment }}</a>
              <!--<el-button type="text" size="small" @click="showAuditor(scope.row.id,typeNameId)">分配</el-button>-->
            </template>
          </el-table-column>
      ...
      <el-dialog
        ...
        <div class="gap-40">
          ...
          <el-radio-group v-model="processType" @change="radioChange">
            <div class="space-20"><el-radio :label="2">公示<el-checkbox v-if="checkFlag" class="gap-40" v-model="checked">开放质疑</el-checkbox></el-radio></div>
...
<script>
  import dataReporting from 'src/pages/auditchanges/auditManage/toAuditManagement/enterprisesToDeclareTabs/tabs/dataReporting'
//引入列表数据
  import {enterprisesToDeclareList} from 'src/axios/auditchanges/toAuditManagement/enterprisesToDeclareTabs'
  ...
  export default {
    ...
    created() {
      //初始化列表
      if(this.$store.state.resDataObj['1']){
        this.resData = this.$store.state.resDataObj['1']
      }
      this.select();
    },
    methods: {
      //让申报数据显示
      showDataReporting(){
      this.dataReportingVisible = true;
      this.$nextTick(()=>{
        this.$refs.dataReporting.select()
      })
      },
      //条件查询
      select(itemVisible) {
        ...
        enterprisesToDeclareList(resData).then(res=>{
      ...
      handleSizeChange(val) {
        ...
        this.select();
        this.$store.state.resDataObj['1'] = this.resData;
      },
      ...
      setProjectNum(rowId,rowName,processTYpe){//设置批次数量
        ...
        this.$router.push('enterprisesToDeclareTabsBatchSet/'+rowId+'/'+rowName+'/'+processTYpe)
      },
      showAuditor(rowId){//审核员分配
        this.$router.push('enterprisesToDeclareTabsDistribution/'+rowId)
      },
