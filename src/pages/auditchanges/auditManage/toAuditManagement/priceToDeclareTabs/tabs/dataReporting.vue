...
<script>
  //引入列表数据
  import {priceToDeclareBatchAdd,priceToDeclareInsert,priceToDeclareCount,priceToDeclareInsertAll} from 'src/axios/auditchanges/toAuditManagement/priceToDeclareTabs'
...
//      this.list()
    ...
    methods: {
      ...
      list() {
        ...
        priceToDeclareBatchAdd(resData,selectQuery).then(res=>{
      ...
      selectCreatedBatch(selectedDataId){//选择创建批次
        ...
        priceToDeclareInsert(idForm).then(res=>{
          // console.log(res,'res......')
          if(res.success){
            this.$message({
              message:'成功',
      ...
      allCreatedBatch(){//全选创建批次
        priceToDeclareInsertAll().then(res=>{
          // console.log(res,'res......')
          if(res.success){
            this.$message({
              message:'保存成功',
