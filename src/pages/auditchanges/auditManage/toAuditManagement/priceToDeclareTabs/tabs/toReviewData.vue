...
<script>
  import dataReporting from 'src/pages/auditchanges/auditManage/toAuditManagement/priceToDeclareTabs/tabs/dataReporting'
  //引入列表数据
  import {priceToDeclareList} from 'src/axios/auditchanges/toAuditManagement/priceToDeclareTabs'
  ...
  export default {
    ...
    //初始化函数
    created() {
      if(this.$store.state.resDataObj['3']){
        this.resData = this.$store.state.resDataObj['3']
      }
      //初始化列表
      this.select()
    },
    methods: {
      ...
      select() {
        ...
        priceToDeclareList(resData).then(res=>{
      ...
      handleSizeChange(val) {
        ...
        this.select();
       this.$store.state.resDataObj['3'] =  this.resData
      ...
      setProjectNum(rowId,rowName,processType){//设置批次数量
        ...
        this.$router.push('priceToDeclareTabsBatchSet/'+rowId+'/'+rowName+'/'+processType)
      },
      showAuditor(rowId){//审核员分配
        this.$router.push('priceToDeclareTabsDistribution/'+rowId)
      },
