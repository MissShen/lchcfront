...
<script>
  import dataReporting from 'src/pages/auditchanges/auditManage/toAuditManagement/enterpriseChangeTabs/tabs/dataReporting'
  //引入列表数据
  import {enterpriseChangeList} from 'src/axios/auditchanges/toAuditManagement/enterpriseChangeTabs'
  ...
  export default {
    ...
    //初始化函数
    created() {
      if(this.$store.state.resDataObj['4']){
        this.resData = this.$store.state.resDataObj['4']
      }
      //初始化列表
      this.select()
    },
    methods: {
      ...
      select() {
        ...
        enterpriseChangeList(resData).then(res=>{
      ...
      handleSizeChange(val) {
        ...
        this.select();
        this.$store.state.resDataObj['4'] = this.resData;
      ...
      setProjectNum(rowId,rowName,processType){//设置批次数量
        ...
        this.$router.push('enterpriseChangeTabsBatchSet/'+rowId+'/'+rowName+'/'+processType)
      },
      showAuditor(rowId){//审核员分配
        this.$router.push('enterpriseChangeTabsDistribution/'+rowId)
      },
