...
<script>
  import dataReporting from 'src/pages/auditchanges/auditManage/toAuditManagement/productDeclarationTabs/tabs/dataReporting'
  //引入列表数据
  import {productDeclarationTabsList,setUpPrompt,setUp} from 'src/axios/auditchanges/toAuditManagement/productDeclarationTabs'
  ...
  export default {
    ...
    //初始化函数
    created() {
      if(this.$store.state.resDataObj['2']){
        this.resData = this.$store.state.resDataObj['2']
      }
      //初始化列表
      this.select()
    },
    methods: {
      ...
      select() {
        ...
        productDeclarationTabsList(resData).then(res=>{
          // console.log(res,'resdfsadsfadsfasdf')
          if(res.success){
            this.resData = res.data;
            if(this.resData.pageNum===0){
              this.resData.pageNum=1
            }
            // this.resData.list.map(item=>item.processType=2)
          }else{
      ...
      handleSizeChange(val) {
        ...
        this.select()
        this.$store.state.resDataObj['2'] = this.resData;
      ...
      setProjectNum(rowId,rowName,processTYpe){//设置批次数量
        ...
        this.$router.push('productDeclarationTabsBatchSet/'+rowId+'/'+rowName+'/'+processTYpe)
      },
      showAuditor(rowId){//审核员分配
        this.$router.push('productDeclarationTabsDistribution/'+rowId)
      },
