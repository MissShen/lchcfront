      ...
      <!-- 表格查询表单 -->
        <el-form ref="query" :model="query" label-position="right" size="small" :inline="true" class="table-form">
            <el-form-item class="width-160">
                <el-input size="small" v-model="query.productName" placeholder="产品名称" @keyup.enter.native="searchEnterFun"></el-input>
           </el-form-item>
          <el-form-item class="width-160">
                <el-input size="small" v-model="query.dosageForm" placeholder="剂型" @keyup.enter.native="searchEnterFun"></el-input>
            </el-form-item>
          <el-form-item class="width-160">
                <el-input size="small" v-model="query.spec" placeholder="规格" @keyup.enter.native="searchEnterFun"></el-input>
            </el-form-item>
          <el-form-item>
...
<script>
  //引入列表数据
  import {productDeclarationBatchAdd,productDeclarationInsert,productDeclarationCount,productDeclarationInsertAll} from 'src/axios/auditchanges/toAuditManagement/productDeclarationTabs'
...
//      this.select()
    ...
    methods: {
      ...
      select() {
        ...
        productDeclarationBatchAdd(resData,selectQuery).then(res=>{
      ...
      selectCreatedBatch(selectedDataId){//选择创建批次
        ...
        console.log(idForm.idList);
        // let dataReportingVisible = false;
        productDeclarationInsert(idForm).then(res=>{
          console.log(res,'res......');
          if(res.success){
            this.$message({
              message:'成功',
      ...
      allBatch(){//全选创建批次
        productDeclarationCount().then(res=>{
          console.log(res,'res......')
          if(res.success){
      ...
      allCreatedBatch(){//全选创建批次
        productDeclarationInsertAll().then(res=>{
          console.log(res,'res......');
          if(res.success){
            this.$message({
              message:'保存成功',
