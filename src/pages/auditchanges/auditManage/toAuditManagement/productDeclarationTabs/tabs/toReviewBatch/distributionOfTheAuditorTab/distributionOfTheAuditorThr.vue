  ...
  <div >
    <div class="tableset">
      <el-table
        ...
        <el-table-column label="已分配数量"align="center">
          <template slot-scope="scope">
        ...
        <el-table-column label="分配数量" align="center">
          <template slot-scope="scope">
            <el-input-number v-model="scope.row.toBeuserCount" controls-position="right" @change="calculation"></el-input-number>
          </template>
        </el-table-column>
...
<script>
  import {productToDeclareAuditorList,queryBaseinfoauditList} from 'src/axios/auditchanges/toAuditManagement/productDeclarationTabs'
  import {saveAuditor} from 'src/axios/auditchanges/toAuditManagement/productDeclarationTabs'
  export default {
    //初始化函数
    name: "distribution-of-the-auditor-thr",
    props:{
      projectId:{
        type:String,
        required:true
    ...
    methods: {
      //条件查询
      select() {
        productToDeclareAuditorList(this.projectId,this.activeName).then(res=>{
          console.log(res,'res一审');
          if(res.data.success){
            // if(res.data.rightBackJson.auditorDomainList.length>0&&this.allData.length>0){
            //    this.tableData =res.data.rightBackJson.auditorDomainList;
            //  }else if(res.data.rightBackJson.auditorDomainList.length>0){
            //    this.tableData = res.data.rightBackJson.auditorDomainList;
            //  }else{
            //   this.tableData =this.allData[this.activeName-1];
            //   console.log(this.tableData,'table')
            // }
            console.log(this.allData.length,'this.allData.length1');
            console.log(this.allData,'this.allData1');
            console.log(this.allData[this.activeName-1],'this.allData[this.activeName-1]1');
      ...
      handleTabel(){
        ...
        this.distributionOfTheAuditorNum.tableData=this.tableData;
        this.distributionOfTheAuditorNum.activeName = this.activeName;
        // console.log(this.tableData,'asdfasd')
        // this.tableData.map(item=>item.toBeuserCount?item.toBeuserCount:0);
