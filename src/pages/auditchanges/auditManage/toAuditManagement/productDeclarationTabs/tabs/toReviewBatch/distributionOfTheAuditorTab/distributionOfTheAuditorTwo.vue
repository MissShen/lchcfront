...
<script>
  ...
  export default {
    //初始化函数
    name: "distribution-of-the-auditor-two",
    props:{
      projectId:{
        type:String,
        required:true
    ...
    methods: {
      ...
      select() {
        productToDeclareAuditorList(this.projectId,this.activeName).then(res=>{
          ...
          if(res.data.success){
            // console.log(this.allData.length,'this.allData.length2');
            // console.log(this.allData,'this.allData2');
            // console.log(this.allData[this.activeName-1],'this.allData[this.activeName-1]2');
      ...
      handleGoEdit () {
        this.showForm = true;
        console.log(this.allData.length,'length')
