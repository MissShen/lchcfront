  ...
  <div class=" padding10">
    <div class="tableset">
      <el-table
        ...
        <el-table-column label="已分配数量" prop="userCount" header-align="center" align="center">
          <template slot-scope="scope">
            <span>{{ scope.row.userCount||0 }}</span>
          </template>
        </el-table-column>
        <el-table-column label="分配数量" header-align="center" align="center">
          <template slot-scope="scope">
            <el-input-number v-model="scope.row.toBeuserCount" size="small" @change="calculation(scope.row.toBeuserCount)" :min="0"></el-input-number>
          </template>
        </el-table-column>
...
<script>
  // import {priceChangeAuditorList} from 'src/axios/auditchanges/toAuditManagement/priceChangeTabs'
  import {productChangeAuditorList} from 'src/axios/auditchanges/toAuditManagement/productChangesTabs'
  import {queryPriceauditList} from 'src/axios/auditchanges/toAuditManagement/priceToDeclareTabs'
  import {saveAuditor} from 'src/axios/auditchanges/toAuditManagement/productDeclarationTabs'
  export default {
    //初始化函数
    name: "distribution-of-the-auditor-one",
    props:{
      projectId:{
        type:String,
        required:true
    ...
    methods: {
      //条件查询
      select() {
        productChangeAuditorList(this.projectId,this.activeName).then(res=>{
          console.log(res,'res一审')
          if(res.success){
            if(this.allData.length>0&&!!this.allData[this.activeName-1]&&this.saveFlag=='1'){
