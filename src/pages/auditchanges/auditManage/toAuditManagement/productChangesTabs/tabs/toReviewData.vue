...
<script>
  import dataReporting from 'src/pages/auditchanges/auditManage/toAuditManagement/productChangesTabs/tabs/dataReporting'
  //引入列表数据
  import {productChangeList} from 'src/axios/auditchanges/toAuditManagement/productChangesTabs'
  ...
  export default {
    ...
    //初始化函数
    created() {
      if(this.$store.state.resDataObj['5']){
        this.resData = this.$store.state.resDataObj['5']
      }
      //初始化列表
      this.select()
    },
    methods: {
      ...
      select() {
        ...
        productChangeList(resData).then(res=>{
      ...
      handleSizeChange(val) {
        ...
        this.select();
        this.$store.state.resDataObj['5'] = this.resData;
      ...
      setProjectNum(rowId,rowName,processType){//设置批次数量
        ...
        this.$router.push('productChangesTabsBatchSet/'+rowId+'/'+rowName+'/'+processType)
      },
      showAuditor(rowId){//审核员分配
        this.$router.push('productChangesTabsDistribution/'+rowId)
      },
