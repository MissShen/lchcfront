...
<script>
  import {productChangeBatchAdd,productChangeInsert,productChangeCount,productChangeInsertAll} from 'src/axios/auditchanges/toAuditManagement/productChangesTabs'
...
//      this.select()
    ...
    methods: {
      ...
      selectCreatedBatch(selectedDataId){//选择创建批次
        ...
        productChangeInsert(idForm).then(res=>{
          // console.log(res,'res......')
          if(res.success){
            this.$message({
              message:'成功',
      ...
      allCreatedBatch(){//全选创建批次
        productChangeInsertAll().then(res=>{
          // console.log(res,'res......')
          if(res.success){
            this.$message({
              message:'保存成功',
