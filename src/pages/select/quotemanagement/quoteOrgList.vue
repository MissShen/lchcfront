    ...
    <div class="tablebox">
      ...
      <div class="tableset">
        <el-table
          ...
          <el-table-column label="投标价格(元)">
            <template slot-scope="scope">
              <span>{{ scope.row.productName }}</span>
            </template>
          </el-table-column>
          <el-table-column label="查看同组报价">
            <template slot-scope="scope">
              <span>{{ scope.row.productName }}</span>
            </template>
          </el-table-column>
...
<script>
  ...
  // 引入用户相关请求  引自js文件
  import {selectQuoteOrgList} from 'src/axios/select/quotemanagement/quoteOrg'
  ...
  export default {
    ...
    methods: {
      ...
      //  获取用户分页列表
      list () {
        selectQuoteOrgList(this.resData, this.query).then(res => {
          console.log(res);
