    ...
    <div class="tablebox">
      ...
      <div class="tableset">
        ...
        >
          ...
          <el-table-column label="上报机构" prop="BUYER_ORGNAME" sortable="custom">
          ...
          <el-table-column label="产品信息" width="300">
            <template slot-scope="scope">
              <el-tooltip placement="top" :enterable="false">
                <div slot="content">
                  {{getSpecHints(scope.row.bakMedicalSpec,scope.row.bakStandSpecRate,scope.row.bakMedicalMode,scope.row.bakSpecUnit,scope.row.warpName)}}<br/>
                  {{scope.row.manufactureName}}
                </div>
                <div class="el-tooltip-box">
                  <div class="text-inline">
                    {{ scope.row.bakMedicalSpec}} × {{ scope.row.bakStandSpecRate}} {{ scope.row.bakMedicalMode}} / {{scope.row.bakSpecUnit}}&nbsp;&nbsp;
              ...
              <el-table-column label="单价" width="200" prop="PRICE" sortable="custom" >
                <template slot-scope="scope">
...
<script>
  ...
  // 引入数据请求
  import {showFttBuyer,exportFttEXL} from 'src/axios/trade/assist/assistConfig'
  ...
  export default {
    ...
    methods: {
      ...
      exportExcel(){
        exportFttEXL(this.query).then(res => {
      ...
      list () {
        ...
        showFttBuyer(this.resData, this.query).then(res => {
