  ...
  <div>
    ...
    <div class="tablebox">
      ...
      <div class="tableset">
        <el-table
          ...
          <el-table-column  label="产品信息" min-width="200" prop="h.bak_medical_spec" sortable="top" >
            <template slot-scope="scope">
              <el-tooltip placement="top" :enterable="false">
                <div slot="content">
                  <span class="text-inlie"  v-if="scope.row.wrapName != '空' && scope.row.wrapName != null">规格包装: {{ scope.row.bakMedicalSpec}} / {{ scope.row.bakUseUnit }} 包材: {{scope.row.wrapName}}</span>
                  <span class="text-inlie"  v-if="scope.row.wrapName == '空' || scope.row.wrapName == null">规格包装: {{ scope.row.bakMedicalSpec}} / {{ scope.row.bakUseUnit }} </span>
                  <br>{{ scope.row.bakProducerShortname}}
                </div>
                <div class="el-tooltip-box">
                  <div class="text-inline">
                    <span class="text-inlie"  v-if="scope.row.wrapName != '空' && scope.row.wrapName != null"> {{ scope.row.bakMedicalSpec}} / {{ scope.row.bakUseUnit }} &nbsp;&nbsp; {{scope.row.wrapName}}</span>
                    <span class="text-inlie"  v-if="scope.row.wrapName == '空' || scope.row.wrapName == null"> {{ scope.row.bakMedicalSpec}} / {{ scope.row.bakUseUnit }} </span>
                  </div>
                  <div class="text-inline" >
                    {{ scope.row.bakProducerShortname}}
                  </div>
                </div>
              </el-tooltip>
            </template>
        ...
        <!-- <el-table-column label="产品名称" prop="h.BAK_PRODUCT_NAME" sortable="custom" show-overflow-tooltip>
            <template slot-scope="scope">
              <span>{{ scope.row.bakProductName }}</span>
            </template>
          </el-table-column>
          <el-table-column label="商品名" prop="h.BAK_TRADE_NAME" sortable="custom">
            <template slot-scope="scope">
              <span>{{ scope.row.bakTradeName }}</span>
            </template>
          </el-table-column>
          <el-table-column label="剂型" prop="h.BAK_MEDICAL_MODE" sortable="custom" show-overflow-tooltip>
            <template slot-scope="scope">
              <span>{{ scope.row.bakMedicalMode }}</span>
            </template>
          </el-table-column>
          <el-table-column label="规格" prop="h.BAK_MEDICAL_SPEC" sortable="custom" show-overflow-tooltip>
            <template slot-scope="scope">
              <span>{{ scope.row.bakMedicalSpec }}</span>
            </template>
          </el-table-column>-->
          <el-table-column label="转换比" prop="h.BAK_STAND_SPEC_RATE" sortable="custom" width="80">
            <template slot-scope="scope">
              <span>{{ scope.row.bakStandSpecRate }}</span>
            </template>
          </el-table-column>
          <!--<el-table-column label="单位" prop="h.BAK_USE_UNIT" sortable="custom" width="60">
            <template slot-scope="scope">
              <span>{{ scope.row.bakUseUnit }}</span>
            </template>
          </el-table-column>
          <el-table-column label="包材" prop="h.WRAP_NAME" sortable="custom" show-overflow-tooltip>
            <template slot-scope="scope">
              <span>{{ scope.row.wrapName }}</span>
            </template>
          </el-table-column>
          <el-table-column label="生产企业" prop="h.BAK_PRODUCER_SHORTNAME" sortable="custom" show-overflow-tooltip>
            <template slot-scope="scope">
              <span>{{ scope.row.bakProducerShortname }}</span>
            </template>
          </el-table-column>-->
          <el-table-column label="单价" prop="h.HIT_PRICE" sortable="custom" width="100">
            <template slot-scope="scope">
...
<script>
  ...
  // 引入数据请求
  import {viewProductList,addPurchase} from 'src/axios/trade/assist/assistConfig'
  ...
  export default {
    ...
    methods: {
      ...
      goAdd(){
        ...
        addPurchase(this.query).then(res => {
          this.$message({
            message: res.data,
            type: 'success',
            duration: 1000,
