    ...
    <div class="tablebox">
      <!-- 表格查询表单 -->
        <el-form ref="query" :model="query" label-position="right" size="small" :inline="true" class="table-form">
            ...
            <el-form-item>
                <el-input v-model="query.searchSimpleName" @keyup.enter.native="searchEnterFun"  placeholder="产品名"  size="small"></el-input>
            </el-form-item>
            <el-form-item>
                <el-input v-model="query.searchKey1" @keyup.enter.native="searchEnterFun"  placeholder="商品名"  size="small"></el-input>
            </el-form-item>
            <el-form-item>
      ...
      <div class="tableset">
        <el-table
          ...
          <el-table-column label="剂型" width="100" prop="doseageFormName" sortable="custom" show-overflow-tooltip>
            <template slot-scope="scope">
      ...
      <div class="foot-btn">
        <div class="foot-left">
          ...
          <el-button type="primary" size="small"  @click="reSetSender()">
            更新配送
          </el-button>
...
<script>
  ...
  // 引入用户相关请求
  import {queryWareHouseProductList,deleteWareHouseProductByIds,updateWareHouseProductByIds} from 'src/axios/trade/information/wareHouseList'
  ...
  export default {
    ...
    methods: {
      ...
      list () {
        ...
        queryWareHouseProductList(this.resData, this.query).then(res => {
      ...
      handleGoAdd() {
        ...
        this.$router.push("/trade/messageMaintain/wareHouseProductAddList/" +warehouseId)
      },
      ...
      handleDelete() {
        if(this.deleteDataList.length<1){
          ...
          this.$confirm(`确定删除选中药品?`, '提示', {
            ...
            deleteWareHouseProductByIds(idList).then(res => {
              this.$message({
                message: res.data,
                type: 'success',
                duration: 1000,
      ...
      reSetSender() {
        if(this.deleteDataList.length<1){
          ...
          this.$confirm(`确定更新选中药品的配送?`, '提示', {
            ...
            var idList=[];
            var senderOrgids=[];
            var arr = this.deleteDataList;
            for(var i=0; i<arr.length; i++){
              idList.push(arr[i].id)
              senderOrgids.push(arr[i].senderOrgId)
            }
            updateWareHouseProductByIds(idList,senderOrgids).then(res => {
              this.$message({
                message: res.data,
                type: 'success',
                duration: 1000,
