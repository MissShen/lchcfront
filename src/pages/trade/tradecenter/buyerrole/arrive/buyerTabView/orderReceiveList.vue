        ...
        <el-form ref="query" :model="query" label-position="right" size="small">
          <div class="formrow">
                <span class="date-group">
                  <el-date-picker type="date" size="small" @keyup.enter.native="searchEnterFun" placeholder="开始日期" format="yyyy-MM-dd" value-format="yyyy-MM-dd" v-model="query.searchBenDate"></el-date-picker>
                &nbsp;至&nbsp;
                ...
                <span>
                  <el-input v-model="query.searchSenderName" placeholder="关键字" size="small"></el-input>
                </span>
                <span>
        ...
        <el-table
          ...
          <el-table-column label="操作" width="120">
            <template slot-scope="scope">
               ...
               <a @click="handleGoShow(scope.row.id)" class="operation-btn">到货修改</a>
            </template>
          </el-table-column>
        </el-table>
        <div class="pagebox">
...
<script>
  ...
  // 引入用户相关请求
  import {selectOrderReceiveList} from 'src/axios/trade/buyerrole/arrive/buyerTabView/orderList'
  ...
  export default {
    ...
    methods: {
      ...
      //  获取用户分页列表
      list () {
        selectOrderReceiveList(this.resData, this.query).then(res => {
          this.resData = res.data;
          if(this.resData.pageNum===0){
            this.resData.pageNum=1;
          }
          this.$emit("receive",this.resData.total);
