<template>
  <div>
    ...
    <div class="tablebox">
      ...
      <div class="tableset">
        ...
        -->
          ...
          <el-table-column label="产品名称"  prop="h.bak_product_name" sortable="custom" min-width="200">
            <template slot-scope="scope" class="drugMessage">
              <ul class="ico-zt">
                <li class="ico-left">
          ...
          <el-table-column  prop="obpw.warehouse_name" sortable="custom" label="药库" v-if="this.repositoryFlag" width="120">
            <template slot-scope="scope">
              <span>{{ scope.row.warehouseName }}</span>
            </template>
          </el-table-column>
          <el-table-column label="配送地址" width="150" show-overflow-tooltip>
            <template slot-scope="scope">
              <el-select v-model="scope.row.addressId" placeholder="请选择" size="small" name="addresses">
                <el-option v-for="item in scope.row.addresses" :key="item.id" :label="item.addressAbbr" :value="item.id">
                </el-option>
              </el-select>
            </template>
          </el-table-column>
          <el-table-column  prop="h.hit_price" sortable="custom" label="单价" header-align="left" align="center" width="190">
            <template slot-scope="scope">
              <!--<div>{{ priceFormat(scope.row.hitPrice) }}</div>
              <div class="rangebar">
                <span class="green">{{ priceFormat(scope.row.maxTradePrice) }}</span>
                <span class="rainbow">
                <i class="iconfont ico-sanjiao2" :style="getStyle(scope.row.hitPrice,scope.row.maxTradePrice,scope.row.minTradePrice)"></i></span>
                <span class="red">{{ priceFormat(scope.row.minTradePrice) }}</span>
              </div>-->
              <range-bar :rangeData="[scope.row.hitPrice,scope.row.maxTradePrice,scope.row.minTradePrice,1]"></range-bar>
            </template>
          </el-table-column>
          <el-table-column label="配送企业" min-width="130">
            <template slot-scope="scope">
          ...
          <el-table-column label="操作" width="140">
            <template slot-scope="scope" style="text-align:left">
              <div class="handle-zt">
                <a class="iconfont buy-btn" style="width: 120px;" @click="handleSavePurchase(scope.row.recordId, scope.row.value,scope.row.addressId,scope.row.requestQty,scope.row.warehouseId,scope.row.warehouseName)">加入采购计划</a>
              </div>
            </template>
          </el-table-column>
        </el-table>
    ...
    <div class="foot-btn">
      <a class="shop-btn" @click = "handleMyShopCar()">
...
<script>
  ...
  // 引入用户相关请求
  import {insertPlanCar,findCarTotal,medicineList} from 'src/axios/trade/plan/planList'
    ...
    methods: {
      ...
      handleAddMinus(newVal, oldVal){
        if(!isNumber(newVal)){
          ...
          //TODO
//          updatePurchaseItemQty(this.curentMode.requestQty,this.curentMode.purchaseItemId,this.curentMode.purchaseId,this.curentMode.repositoryId).then(res => {
//            this.$message({
//              message: res.data,
//              type: 'success',
//              duration: 1000,
...
//          });
      ...
      handleSavePurchase(recordId, senderId, addressId,requestQty,repositoryId,repositoryName){
        if(null == senderId){
          this.$alert('请选择配送', '提示', {
          ...
          insertPlanCar(recordId, senderId,addressId, requestQty,repositoryId,repositoryName).then(res => {
            this.$message({
              message: res.data,
              type: 'success',
              duration: 1000,
