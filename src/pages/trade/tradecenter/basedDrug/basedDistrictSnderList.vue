  ...
  <div class="tablebox">
    ...
    <div class="tableset">
      ...
      >
        ...
        <el-table-column label="配送企业名称" prop="co.name" sortable="custom">
          <template slot-scope="scope">
            {{ scope.row.name }}
          </template>
        </el-table-column>
        <el-table-column label="配送企业简称" prop="co.abbr" sortable="custom">
          <template slot-scope="scope">
    ...
    <div class="foot-btn">
      <div class="foot-left">
        <el-button type="danger" size="small" @click="rem()">移除</el-button>
      </div>
    </div>
  </div>
  <!-- ========================== 主内容end ========================= -->
...
<script>
  ...
  // 引入用户相关请求
  import {selectDistrictBasedDrugList} from 'src/axios/trade/basedDrug/basedDrugSernderList'
  import {districtRemoveDrugCatalog} from 'src/axios/trade/basedDrug/basedDrugSernderList'
  import {priceFormat, keyBoardEnter} from "src/utils";

  export default {
    name: 'basedDistrictSnderList',
    data() {
      return {
    ...
    methods: {
      ...
      rem() {
        if (this.multipleSelection.length < 1) {
          ...
          this.$confirm(`确定移除?`, '提示', {
            ...
            districtRemoveDrugCatalog(ids).then(res => {
              this.$message({
                message: "成功！",
                type: 'success',
                duration: 1000,
      ...
      list() {
        ...
        selectDistrictBasedDrugList(this.resData, this.query).then(res => {
