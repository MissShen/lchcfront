    ...
    <div class="tablebox">
      ...
      <div class="tableset">
        <el-table
          ...
          <el-table-column label="采购单编号" width="140" prop="p.purchase_Code" sortable="custom" show-overflow-tooltip>
            <template slot-scope="scope">
              <span>{{ scope.row.purchaseCode}}</span>
            </template>
          </el-table-column>
          <el-table-column label="买方机构" prop="p.bak_Buyer_Easy" sortable="custom">
            <template slot-scope="scope">
              {{ scope.row.bakBuyerEasy == null ?  scope.row.bakBuyerName:scope.row.bakBuyerEasy }}
            </template>
          </el-table-column>
          <el-table-column label="创建时间" prop="p.create_date" sortable="custom" width="150" align="center">
            <template slot-scope="scope">
              <span>{{ scope.row.createDate }}</span>
            </template>
          </el-table-column>
          <el-table-column label="品种数量" prop="p.REQUEST_TOTAL" sortable="custom" header-align="left" align="right" width="100">
            <template slot-scope="scope">
              <span>{{ scope.row.purchaseItemNum }}</span>
            </template>
          </el-table-column>
          ...
          <el-table-column label="采购类别" prop="p.flag" align="center" sortable="custom" width="100">
            <template slot-scope="scope">
              <span v-if="scope.row.flag==0">{{ getFlag(scope.row.flag) }}</span>
              <span v-if="scope.row.flag==1" class="green">{{ getFlag(scope.row.flag) }}</span>
              <span v-if="scope.row.flag==2" class="orange">{{ getFlag(scope.row.flag) }}</span>
              <span v-if="scope.row.flag==3" class="blue">{{ getFlag(scope.row.flag) }}</span>
            </template>
          </el-table-column>
          <el-table-column label="采购单别名" prop="p.BY_NAME" sortable="custom" >
            <template slot-scope="scope">
                <span @click="dataBackFillGoods"  class="flo-right" v-if="scope.row.purchaseId != cellIndex" :title="scope.row.byName">
                  {{ scope.row.byName }}<i class="el-icon-edit"></i>
                </span>
          ...
          <el-table-column label="状态" prop="p.PURCHASE_STATE" sortable="custom" align="center" width="100">
            <template slot-scope="scope">
          ...
          <el-table-column label="操作" width="100">
            <template slot-scope="scope">
              <div class="operates">
                ...
                <!--<a class="green" v-if="scope.row.purchaseState == 2" @click="skipOrdOrderItem(scope.row.purchaseCode)">订单</a>-->
                  <a class="blue-bord" type="text" size="small" @click="checkOne(scope.row.purchaseId)"
                  ...
                  <a v-if="scope.row.isEdit != '1'" class="green" type="text" size="small" @click="showInfo(scope.row.purchaseId)">详情</a>
                </span>
                <!--<span>-->
                  <!--<a class="green" type="text" size="small" v-if="scope.row.templateState == '0' && scope.row.roleCode !='manage'" @click="openDialog(scope.row.purchaseId,'1')">设置模板</a>-->
                  <!--<a class="green" type="text" size="small" v-if="scope.row.templateState == '1'" @click="cancelTemPlat(scope.row.purchaseId,'0')">取消模板</a>-->
                <!--</span>-->
                <!-- <a class="operation-btn" type="text" size="small" @click="getPrintData(scope.row.purchaseId)">打印</a>-->
                <!--<span>-->
                  <!--<a class="green" type="text" size="small" @click="exportShowInfoExcel(scope.row.purchaseId)">导出</a>-->
                <!--</span>-->
                <!--<span>-->
                  <!--<a class="green" type="text" size="small" @click="copySave(scope.row.purchaseId)" v-if="scope.row.roleCode !='manage'">复制</a>-->
                <!--</span>-->
                <!--<span>-->
                  <!--<a class="green" type="text" size="small" @click="showInfo(scope.row.purchaseId)">详情</a>-->
                <!--</span>-->
                <!--<span>-->
                  <!--<a class="red" type="text" size="small" @click="deletePurchase(scope.row.purchaseId)"-->
                     <!--v-if="scope.row.purchaseState=='0' || scope.row.purchaseState=='3' || scope.row.purchaseState=='6'" >-->
                    <!--删除-->
                  <!--</a>-->
                <!--</span>-->
                <!--<span>-->
                  <!--<a class="red" type="text" size="small" @click="backPurchase(scope.row.purchaseId)"-->
                     <!--v-if="scope.row.isBack =='1'" >-->
                    <!--&lt;!&ndash;(scope.row.purchaseState=='1' && (roleCode == 'doctor' || roleCode == 'service'))&ndash;&gt;-->
                    <!--&lt;!&ndash;|| (scope.row.purchaseState=='5' && roleCode == 'serviceCenter')&ndash;&gt;-->
                    <!--撤销-->
                  <!--</a>-->
                  <!--<a class="red" type="text" size="small" @click="handleSendbackPurchase(scope.row.purchaseId)"-->
...
//      getRoleCode();
    ...
    methods: {
      ...
      exportExcel(){
       ...
       /*     if (this.roleCode == "serviceCenter" || this.roleCode == "director") {
              if (this.query.searchPurchaseState == undefined || this.query.searchPurchaseState == '') {
                state = '1';
                this.checkedAll = true;
              }
            } else if (this.roleCode == "manage") {
              if (this.query.searchPurchaseState == undefined) {
                state = '5';
                this.checkedAll = true;
              }
            }*/
...
//          }
          this.$nextTick(()=>{
            queryFirstPlanList(this.resData, this.query).then(res => {
              ...
              if(this.startState.length===0){
                this.startState=this.query.searchPurchaseState
                this.startCount=this.resData.total
              }
              this.$emit("planList", this.startState==this.query.searchPurchaseState?this.resData.total:this.startCount);
            });
          })
        });
...
//              })
      ...
      commit(el){
        let title=$("#templateContent").val();
        this.template.byName=title;
