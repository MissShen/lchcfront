  ...
  <div class="tablebox">
      ...
      <el-form ref="query" :model="query" size="small" :inline="true" class="demo-form-inline table-form">
        ...
        <el-form-item>
          <el-date-picker type="date" placeholder="历史采购开始" v-model="query.searchBenDate"
                          ...
                          format="yyyy-MM-dd" value-format="yyyy-MM-dd" size="small" class="width-120"></el-date-picker>
          &nbsp;至&nbsp;
        ...
        <el-form-item>
          ...
          <el-button @click="returnEx()" size="small">返回</el-button>
        </el-form-item>
      </el-form>
      ...
      <div class="tableset">
        <el-table ref="multipleTable" :data="resData.list" tooltip-effect="dark"
          ...
          <el-table-column label="产品信息" prop="h.bak_medical_spec" sortable="custom">
          ...
          <el-table-column prop="enterprise" label="配送企业" width="150" show-overflow-tooltip>
            <template slot-scope="scope">
              <el-select v-model="scope.row.firstSenderId" placeholder="请选择" size="small" name="sendBusiness" @change="changeSendBusiness(scope.row,scope.row.sendBusiness,scope.row.firstSenderId)">
                <el-option v-for="item in scope.row.sendBusiness" :key="item.senderId" :label="item.senderEasy" :value="item.senderId">
                </el-option>
              </el-select>
            </template>
          </el-table-column>
          <el-table-column label="数量" width="150" show-overflow-tooltip>
            <template slot-scope="scope">
        <!--      <el-input-number size="small" v-model="scope.row.requestQty" name="requestQty"
            ...
            <!--  <el-input size="small" v-model="scope.row.requestQty" name="requestQty"  @change="handleRequestQtyChange" ></el-input>-->


              <template v-if="scope.row.purchaseProductId == '0' && scope.row.enableFlag == '1'">
                <el-input size="small" v-model="scope.row.requestQty" name="requestQty" :ref="'requestQty'+scope.$index"
                          @keyup.up.native="keyBoardUp(scope.$index)" @keyup.down.native="keyBoardDown(scope.$index)"
                          @keyup.enter.native="handleSavePurchase(scope.row,scope.row.recordId, scope.row.firstSenderId,scope.row.addressId,scope.row.requestQty,scope.row.warehouseId,scope.row.warehouseName,scope.row.priceSource,scope.row.productId)"></el-input>
              </template>
          ...
          <el-table-column prop="remark" label="操作" width="110">
            <template slot-scope="scope" style="text-align:left">
              <div class="handle-zt">
                <a class="iconfont buy-btn" v-if="scope.row.purchaseProductId == '0' && scope.row.enableFlag == '1'"@click="handleSavePurchase(scope.row,scope.row.recordId, scope.row.firstSenderId,scope.row.addressId,scope.row.requestQty,scope.row.warehouseId,scope.row.warehouseName,scope.row.priceSource,scope.row.productId)">加入</a>
                <a class="iconfont buy-btn gray" v-if="scope.row.purchaseProductId == '0' && (scope.row.enableFlag == '2'|| scope.row.enableFlag == '3'|| scope.row.enableFlag == '4' || scope.row.enableFlag == '5')">加入</a>
                <a class="iconfont buy-btn orange js-addcart" v-if="scope.row.purchaseProductId != '0'" @click="handleDeletePurchase(scope.row.productId)">移除</a>
              </div>
            </template>
          </el-table-column>
        </el-table>
  ...
  export default {
    ...
    methods: {
      ...
      list() {
        ...
        templateMedicineList(this.resData, this.query, this.purchaseId).then(res => {
          console.log(res);
          ...
          findApproveCarTotal("0", this.purchaseId).then(res => {
            ...
            console.log(res.data.purchaseId);
      ...
      handleSavePurchase(entry, recordId, senderId, addressId, requestQty, repositoryId, repositoryName,priceSource,productId) {
        ...
        if (null == addressId) {
          ...
          return false
            ...
            this.$confirm(
              ...
              addMedicineTemplate(this.purchaseId, recordId, senderId, addressId, requestQty, repositoryId, repositoryName,priceSource,productId).then(res => {
                this.$message({
                  message: res.data,
                  type: 'success',
                  duration: 1000,
                  onClose: () => {
                    this.$router.push("/trade/manage/purchaseTemplateInfo/"+ this.purchaseId)
                  }
                })
              })
            })
          ...
          addMedicineTemplate(this.purchaseId, recordId, senderId, addressId, requestQty, repositoryId, repositoryName,priceSource).then(res => {
            this.$message({
              message: res.data,
              type: 'success',
              duration: 1000,
...
//        }
      ...
      handleDeletePurchase(productId){
        deleteMedicineTemplate(productId,this.purchaseId).then(res => {
          this.$message({
            message: res.data,
            type: 'success',
            duration: 1000,
