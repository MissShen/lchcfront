<template>
  <el-dialog
    ...
    <span slot="footer">
      <el-button size="small" type="primary" @click="handleDelete()">新增关联</el-button>
...
<script>
  ...
  import {queryNoRelatedOrg, insertRelatedOrg} from 'src/axios/datacenter/orgdata/sell/seller'
  ...
  export default {
    ...
    methods: {
      ...
      metaList() {
        ...
        queryNoRelatedOrg(this.dataForm.parentId, this.query, this.resData).then(res => {
          console.log("==>", res);
      ...
      // 获取已关联企业列表
      list () {
        queryNoRelatedOrg(this.id ,this.query, this.resData).then(res => {
      ...
      handleDelete() {
        if(this.deleteDataList.length<1){
          ...
          this.$confirm(`确定关联选择的企业?`, '提示', {
            ...
            insertRelatedOrg(ids.join(',').toString(), this.id).then(res=>{
              this.$message({
                message: res.data,
                type: 'success',
                duration: 1000,
