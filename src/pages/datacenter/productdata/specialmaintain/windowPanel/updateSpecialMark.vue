<template>
  <el-dialog
    ...
    <span slot="footer">
      <el-button size="small" type="primary" @click="buttonForm(`form`)">保存</el-button>
...
<script>
  ...
  // 引入用户相关请求
  import {provinceList,updateSpecial} from 'src/axios/datacenter/productdata/specialmaintain/specialmaintain';
    ...
    methods: {
      ...
      provinceList() {
        ...
        for(var i=0;i<this.provinces.length;i++){
          if(this.form.provinceId==this.provinces[i].nameChn){
            this.form.provinceId=this.provinces[i].id;
          }
          var jo={};
      ...
      buttonForm(form){
        this.$refs[form].validate((valid) => {
          if (valid) {
            updateSpecial(this.form).then(res => {
              this.$message({
              message: res.data,
              type: 'success',
              duration: 1000,
