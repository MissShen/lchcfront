<template>
  <div>
          ...
          <el-row :gutter="24">
            <el-col :span="12">
              <el-form-item label="关联药品：">
                <el-input v-model="form.choicedrugsnum20" class="width-240" disabled="true"></el-input>
                <span>种</span>
                <a @click="drugsListView('0')">选择</a>
              </el-form-item>
            </el-col>
          </el-row>
    ...
    <drugs-list v-if="drugsListVisible" ref="DrugsList" v-bind="{pharmType:this.pharmType,drugIds8:this.form.drugIds8,doseageIds:this.form.doseageIds}" @saveDrugIds20="saveDrugIds20"></drugs-list>
...
<script>
  ...
  import {insertCatAtc} from 'src/axios/datacenter/basedata/catatccategory/catatclist';
    ...
    methods: {
      onSubmit(form) {
        ...
        this.$refs[form].validate((valid) => {
          if (valid) {
            insertCatAtc(this.form).then(res => {
              this.$message({
                message: res.data,
                type: 'success',
                duration: 1000,
      ...
      saveDrugIds20(ids){
        ...
        this.form.choicedrugsnum20= ids.split(',').length;
      },
      drugsListView(id) {
