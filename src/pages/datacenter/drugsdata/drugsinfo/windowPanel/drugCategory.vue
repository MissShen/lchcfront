    ...
    <div class="tableset">
      <el-table
        ...
        <el-table-column label="操作">
          <template slot-scope="scope">
            <div class="operates"><a class="operation-btn" @click="myCallBack(scope.row.CODE, scope.row.ID, scope.row.NAME)">选择</a></div>
          </template>
        </el-table-column>
      </el-table>
    </div>
...
<script>
  ...
  import {drugBigClass} from 'src/axios/datacenter/drugdata/druginfo/druginfo'
  ...
  export default {
    ...
    methods: {
      ...
      metaList() {
        drugBigClass(this.resData, this.query).then(res => {
          this.resData = res.data;
          console.log(res.data,146)
        })
      },
      // 重置
      resetForm() {
