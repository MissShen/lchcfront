...
<script>
  ...
  import {findDrugSynonymInfo, updateDrugSynonymInfo} from 'src/axios/datacenter/drugdata/synonym/synonym'
  ...
  export default {
    ...
    created () {
      ...
      findDrugSynonymInfo(this.id).then(res => {
        this.form = res.data
        console.log("id=", this.id, "this.form=",this.form);
      })
    ...
    methods: {
      onSubmit(formName) {
        this.$refs[formName].validate((valid) => {
          if (valid) {
            updateDrugSynonymInfo(this.form).then(res => {
              this.$message({
                message: res.data,
                type: 'success',
                duration: 1000,
      ...
      synonymDrugsView(id) {
        ...
        this.$nextTick(() => {
          this.$refs.SynonymDrugs.fillData(id);
        })
      },
      handleBack(){
        this.$router.go(-1)
