    ...
    <div class="tablebox">
        ...
        <el-form ref="query" :model="query" label-position="right" :inline="true" size="small" class="table-form">
            <el-form-item>
              <el-select v-model="query.searchType" placeholder="搜索条件" size="small">
                ...
                <el-option label="国家编码" value="3"></el-option>
              </el-select>
            </el-form-item>
      ...
      <div class="tableset">
        <el-table
          ...
          <el-table-column sortable="custom" prop="code" label="国家编码" width="200">
            <template slot-scope="scope">
              <span>{{ scope.row.code }}</span>
            </template>
          </el-table-column>

          <el-table-column sortable="custom" prop="name_chn" label="国家中文名称">
            <template slot-scope="scope">
              <span>{{ scope.row.nameChn }}</span>
            </template>
          </el-table-column>

          <el-table-column sortable="custom" prop="name_eng" label="国家英文名称" show-overflow-tooltip>
            <template slot-scope="scope">
...
<script>
  ...
  // 引入用户相关请求
  import {countryList, remove} from 'src/axios/datacenter/basedata/region/region'
  ...
  export default {
    ...
    methods: {
      ...
      list () {
        this.$store.state.pageSeach = this.query;
        countryList(this.query, this.resData).then(res => {
      ...
      // 跳转新增页面
      goAdd () {
        this.$router.push("/DATA/basedata/country/add");
      },
      // 跳转详情页面
      goInfo(id) {
        this.$router.push("/DATA/basedata/country/info/"+id);
      },
      // 跳转编辑页面
      goEdit(id) {
        this.$router.push("/DATA/basedata/country/edit/"+id);
      }
    }
  }
</script>
