      ...
      <div class="width-520">
        <el-form :model="from" ref="from" :rules="rules" label-position="left" label-width="120px" class="demo-ruleForm" size="small">
          ...
          <el-form-item label="所在国家：" prop="consignee">
              <el-select v-model="from.county" placeholder="请选择" class="w100">
                <el-option
...
<script>
  ...
  import {countryAllList, addProvince} from 'src/axios/datacenter/basedata/region/region'
    ...
    methods: {
      submitForm(from) {
        this.$refs[from].validate((valid) => {
          if (valid) {
            addProvince(this.from).then(res => {
              this.$message({
                message: res.data,
                type: 'success',
                duration: 1000,
