    ...
    <div class="tablebox">
        ...
        <el-form ref="query" :model="query" label-position="right" :inline="true" size="small" class="table-form">
            <el-form-item>
              <el-select v-model="query.searchType" placeholder="搜索条件"  size="small">
                <el-option label="省份名称" value="1"></el-option>
                <el-option label="省份编码" value="3"></el-option>
              </el-select>
            </el-form-item>
      ...
      <div class="tableset">
        <el-table
          ...
          <el-table-column sortable="custom" prop="code" label="省份编码" width="200">
            <template slot-scope="scope">
              <span>{{ scope.row.code }}</span>
            </template>
          </el-table-column>

          <el-table-column label="所在国家">
            <template slot-scope="scope">
              <span>{{ scope.row.parentName }}</span>
            </template>
          </el-table-column>

          <el-table-column sortable="custom" prop="name_chn" label="省份名称">
            <template slot-scope="scope">
              <span>{{ scope.row.nameChn }}</span>
            </template>
          </el-table-column>

          <el-table-column sortable="custom" prop="abbr" label="省份简称">
            <template slot-scope="scope">
      ...
      <div class="foot-btn">
        <div class="foot-left">
          <el-button type="success" size="small" @click="goAdd">增加</el-button>
...
<script>
  ...
  import {provinceList, remove} from 'src/axios/datacenter/basedata/region/region'
  ...
  export default {
    ...
    methods: {
      ...
      list () {
        this.$store.state.pageSeach = this.query;
        provinceList(this.query, this.resData).then(res => {
      ...
      // 跳转新增页面
      goAdd () {
        this.$router.push("/DATA/basedata/province/add");
      },

      // 跳转详情页面
      goInfo(id) {
        this.$router.push("/DATA/basedata/province/info/"+id);
      },

      // 跳转编辑页面
      goEdit(id) {
        this.$router.push("/DATA/basedata/province/edit/"+id);
      }
