        ...
        <el-form ref="query" :model="query" label-position="right" :inline="true" size="small" class="table-form">
            <el-form-item>
              <el-select v-model="query.searchType" size="small">
                <el-option label="计量单位编码" value="0"></el-option>
                <el-option label="计量单位中文名称" value="1"></el-option>
              </el-select>
            </el-form-item>
          ...
          <el-table-column
            ...
            <template slot-scope="scope">
              <div class="operates"><a class="operation-btn" @click="goInfo(scope.row.id)">查看</a>
                <a @click="goEdit(scope.row.id)" class="operation-btn">修改</a></div>
            </template>
          </el-table-column>
        </el-table>
      </div>
...
<script>
  ...
  import {metric} from 'src/axios/datacenter/basedata/metric/metric'
  import {del} from 'src/axios/datacenter/basedata/metric/metric'
  ...
  export default {
    ...
    methods: {
      ...
      list2 () {
        ...
        this.$store.state.pageSeach = this.query;
        metric(this.query, this.resData).then(res => {
      ...
      goAdd () {
        this.$router.push("/DATA/basedata/measurementunit/add");
      },
      goInfo(id) {
        this.$router.push("/DATA/basedata/measurementunit/info/" + id);
      },
      goEdit(id) {
        this.$router.push("/DATA/basedata/measurementunit/update/" + id);
      }
    }
  }
</script>
