...
<script>
  ...
  // 引入用户相关请求
  import {queryDrugList} from  'src/axios/datacenter/authdata/medicalinsurance/medicalinsurance'
  ...
  export default {
    ...
    methods: {
      ...
      list2(){
        ...
        queryDrugList(this.resData,this.dataForm).then(res => {
      ...
      buttonForm() {
        if (this.assoList.length > 0) {
          this.$confirm('确认选择' + this.assoList.length + '个药品', '确认', {
            ...
            this.visible = false;
          this.$emit('chooseDrugsList', {value: this.assoList})
        })
        } else {
