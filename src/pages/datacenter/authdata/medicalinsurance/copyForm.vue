...
<script>
  ...
  import {insertMedicare} from 'src/axios/datacenter/authdata/medicalinsurance/medicalinsurance';
  ...
  export default {
    ...
    methods: {
      ...
      chooseDosageList(data){
        var existDosageNames=[];
        for(var i=0;i<data.value.length;i++){
          var exist=false;
          for(var j=0;j<this.backDoseageList.length;j++){
            if(data.value[i].code==this.backDoseageList[j].code){
              exist=true;
            }
          }
          if(exist==false){
            ...
            existDosageNames.push(data.value[i].name)
          }
        }
        if(existDosageNames.length>0){
          confirm(existDosageNames.toString()+"已选择");
        }
      ...
      chooseRelateDrugsList(val){
        ...
        for(var i=0;i<val.value.length;i++){
          ...
          for(var j=0;j<this.checkRelatedDrugs.length;j++){
            if(val.value[i].id==this.checkRelatedDrugs[j].id){
              exist=true;
              existName.push(val.value[i].nameChn);
            }
          }
          if(exist==false){
        ...
        if(existName.length>0){
          confirm(existName.toString()+"已选择");
        }
        this.relatedDrugsNum=this.checkRelatedDrugs.length;
        this.form.chooseRelateDrugsId=this.setChooseRelateDrugsId();
      },
