<template>
  <div>
          ...
          <el-row :gutter="24">
            ...
            <el-col :span="12">
              <el-form-item label="通用名：">
                <el-input v-model="form.drugName" disabled="true" class="width-260"></el-input>
              </el-form-item>
            </el-col>
          </el-row>
              ...
              <el-form-item label="相关产品信息：">
               <span>已关联
                  <el-input v-model="form.productNum" class="width-160" placeholder="0" disabled="true"></el-input>
                            种药品</span>
                <a @click="chooseUProduct">已关联</a>
                |
                <a @click="chooseProduct">未关联</a>
              </el-form-item>
            </el-col>
          </el-row>
    ...
    <asso-product-info v-if="productVisible" ref="AssoProductInfo" v-bind="{type:this.form.ptype,orgCode:this.form.orgCode,productId:this.form.id}" @saveProIds = "saveProIds"></asso-product-info>
  </div>
</template>
<script>
  ...
  import {info,update} from 'src/axios/datacenter/authdata/protecteddrug/protecteddrug';
      ...
      this.id = this.$route.params.id || undefined;
        if (this.id) {
          info(this.id).then(res => {
            this.form = res.data;
            this.pno = this.form.productNum;
            this.form.applyD = this.form.applyDate;
            this.form.accreditD = this.form.accreditDate;
          })
        }
    },
    methods: {
