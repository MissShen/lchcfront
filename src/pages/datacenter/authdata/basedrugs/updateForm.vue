<template>
  <div>
          ...
          <el-row :gutter="24">
            <el-col :span="12">
              <el-form-item label="关联药品：">
               <span>已选择
                  <el-input v-model="form.count" class="width-200" placeholder="0" readonly="true"></el-input>
                            种药品</span>
                <a @click="hasAssDrugWindow('HAS')">已关联</a>
                |
                <a @click="notAssDrugWindow('NOT')">未关联</a>
              </el-form-item>
            </el-col>
          </el-row>
    ...
    <common-asso-drugs v-if="assoVisible" ref="CommonAssoDrugs" @chooseAssDrugs="chooseAssDrugs"></common-asso-drugs>
  </div>
</template>
<script>
  ...
  import CommonAssoDrugs from "./windowPanel/assoDrugs";
  ...
  import {delCountryDrug} from 'src/axios/datacenter/authdata/countrybasedrugs/countryBase'
  import {addCountryDrug} from 'src/axios/datacenter/authdata/countrybasedrugs/countryBase'
  import {update} from 'src/axios/datacenter/authdata/countrybasedrugs/countryBase'
  ...
  export default {
    ...
    methods: {
      ...
      chooseAssDrugs(data) {
        ...
        if (data.flag === '已关联') {
          delCountryDrug(this.form.catAuthCountryBase.id, ids.toString()).then(res => {
            this.$message({
              message: res.data,
              type: 'success',
              duration: 1000,
          ...
          addCountryDrug(this.form.catAuthCountryBase.id, ids.toString()).then(res => {
            this.$message({
              message: res.data,
              type: 'success',
              duration: 1000,
      ...
      hasAssDrugWindow() {
        ...
        this.$nextTick(() => {
          this.$refs.CommonAssoDrugs.fillData('HAS', this.form.catAuthCountryBase.id);
        })
      },

      notAssDrugWindow() {
        var data = {id: "", codes: "", ids8: ""};
        data.id = this.form.catAuthCountryBase.id;
        data.ids8 = this.drugsArr.toString();
        data.codes = this.dosageFormArr.toString();
        this.assoVisible = true;
        this.$nextTick(() => {
          this.$refs.CommonAssoDrugs.fillData('NOT', data);
        })
      },
